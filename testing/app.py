...
def get_agentic_rag_app(_retriever, openai_api_key, tavily_api_key):
    ...
    SYS_PROMPT_RAG = """You are an assistant for question-answering Bangladeshi Legal tasks.
             ...
             **IMPORTANT CITATION INSTRUCTIONS:** For each piece of information you use from the context, cite its origin clearly at the end of the relevant sentence or paragraph.
             ...
             - If the URL is 'N/A', cite it as (Source: Web Search - URL N/A).
             - If the source is 'Unknown Source', cite it as (Source: Unknown).
             Combine information logically, but ensure all sources are traceable.
    ...
    # --- MODIFIED format_docs_with_sources ---
    def format_docs_with_sources(docs: List[Document]) -> str:
        """Formats documents for the RAG prompt, including specific sources (filename or URL)."""
        if not docs:
        ...
        for i, doc in enumerate(docs):
            ...
            # --- NEW: Check source type and format accordingly ---
            if source == "Web Search":
                url = doc.metadata.get('url', 'N/A')
                formatted.append(f"--- Context from: Web Search (URL: {url}) ---\n{content}")
            elif source != 'Unknown Source': # Assume it's a filenamef
                formatted.append(f"--- Context from: {source} ---\n{content}")
            else: # Fallback for truly unknown sources
                formatted.append(f"--- Context from: Unknown Source ---\n{content}")
            # --- END NEW ---
        return "\n\n".join(formatted)
